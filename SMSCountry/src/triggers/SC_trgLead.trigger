trigger SC_trgLead on Lead (after insert, after update, after delete) {
    ...
    if(cAct=='Inserted' || cAct=='Updated') {
    	for (Lead a : Trigger.new) {
            //SMSText = 'Lead : '+a.LastName+' '+(a.FirstName!=NULL?a.FirstName+' ':'')+cAct;
            cformat = dftemplate;
            cformat = cformat.replace(tag1, rtag1);
            rtag2 = a.LastName+' '+(a.FirstName!=NULL?a.FirstName+' ':'');
