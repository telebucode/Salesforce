trigger SC_trigContact on Contact (after insert, after update, after delete) {
    ...
    if(cAct=='Inserted' || cAct=='Updated') {
    	for (Contact a : Trigger.new) {
            //SMSText = 'Contact : '+(a.FirstName!=NULL?a.FirstName+' ':'')+a.LastName+' '+cAct;
            cformat = dftemplate;
            cformat = cformat.replace(tag1, rtag1);
            rtag2 = a.FirstName!=NULL?a.FirstName+' ':'';
